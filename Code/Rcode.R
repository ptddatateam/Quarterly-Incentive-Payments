...
#   - All packages used in this process are stored in the folder with this script and will not be updated unless they are tested and saved.
#   - Ian Wesley 6/21/18
#
#   - Other Items:
...
#
#   - Updates 12/6/2018 - Ian Wesley:
#       - An issue was identified where people use an eligible mode in only one direction of their daliy commute. When applying the 60%
#         rule these people would not recieve any incentive. It was decided that if a person has only 2 comutes the 60% rule will not apply.
####
...
# Non-technical 
EtcEmailAddress        <- # ENTER EMAL ADDRESS HERE
...
WorkingDirectory <- "H:/RWorking/QuarterlyIncentivePayments"
...
# This function removes trips so that the remaining trips will pass the 60% rule.  
#    For example: if some carpools to work, then drives alone.  It will remove the drive alone trip so the remaining single carpool trip will get an incentive.  
RemoveTripsNotSubjectTo60PercentRule <- function(InputData, 
  ...
  output <- tryCatch({
    ...
    PersonsInCountiesNotSujbectTo60PercentRule <- c()
    myModesInCountiesNotSujbectTo60PercentRule <- c()
    myModesNotSubjectTo60PercentRule <- c()
    myEligibleModes <- EligibleModes
    ...
    OnlyTwoTripsPerday_TripsToRemove <- NoTripsNotSubjectTo60PercentRule %>% 
      group_by(eMail, TripDate) %>% 
      filter(n() == 2 & !(Mode %in% myEligibleModes))
    
    NoTripsNotSubjectTo60PercentRule <- NoTripsNotSubjectTo60PercentRule %>% 
      anti_join(OnlyTwoTripsPerday_TripsToRemove)
...
####
oldOutput <- read.csv("G:/QuarterlyIncentivePayments/Archive/Output/Archived 16Jul2018 Output.csv", stringsAsFactors = F)

t <- full_join(Output, oldOutput, by = "Email.Address")

t$Sum.of.Total.Incentive.Due.for.Quarter.y[is.na(t$Sum.of.Total.Incentive.Due.for.Quarter.y)] <- 0
t$Sum.of.Total.Incentive.Due.for.Quarter.x[is.na(t$Sum.of.Total.Incentive.Due.for.Quarter.x)] <- 0

t$Underpaid <- t$Sum.of.Total.Incentive.Due.for.Quarter.y - t$Sum.of.Total.Incentive.Due.for.Quarter.x

write.csv(t,  "Draft_FirstQuarterMissingPayments.csv")

sum(t$Underpaid, na.rm = T)
